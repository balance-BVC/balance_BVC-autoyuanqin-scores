...
repo/
└── js/
    └── AutoYuanQin-balance/    ← 完整 AutoYuanQin 脚本（官方 3.7.6 + 个人曲谱）
        ├── main.js
        ├── manifest.json
        ├── settings.json
        ├── README.md
...
3. 点击「更新仓库」，等待完成。
4. 在 Javascript脚本 目录下找到 AutoYuanQin-balance 并订阅，之后即可在调度器中使用。

## 如何添加自己的曲谱

1. 使用 `repo/js/AutoYuanQin-balance/assets/tutorial_file/` 下的制谱器生成 JSON，或手写 `keyboard` 类型键盘谱。
2. 放入 `repo/js/AutoYuanQin-balance/assets/score_file/`，确保 JSON 的 `name` 与文件名一致。
...
- AutoYuanQin 脚本本体来自官方 [babalae/bettergi-scripts-list](https://github.com/babalae/bettergi-scripts-list)（`repo/js/AutoYuanQin-balance`），本仓库在此基础上维护个人曲谱，并重命名为 `AutoYuanQin-balance` 以避免与官方脚本重名冲突。
